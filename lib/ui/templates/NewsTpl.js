  ...
  render() {
    ...
    const renderDates = () => (
      <div>
        ...
        <div>
          {dates.primary.start
            ? formatDate(dates.primary.start, "DD MMMM YYYY", lingo)
            : null}{" "}
          –{" "}
          {dates.primary.end
            ? formatDate(dates.primary.end, "DD MMMM YYYY", lingo)
            : null}
          {dates.primary.opening ? (
            <p>
              {DICT.datesOpening[lingo]}:{" "}
              {formatDate(dates.primary.opening, "DD MMMM YYYY", lingo)}
            </p>
          ) : null}
          {dates.primary.deadline ? (
            <p>
              {DICT.datesClosing[lingo]}:{" "}
              {formatDate(dates.primary.deadline, "DD MMMM YYYY", lingo)}
            </p>
          ) : null}
        </div>
      </div>
